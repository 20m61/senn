...
| PeerSession spec checklist | `pnpm --filter @senn/core test` (`peer-session.test.ts`) — covers every MUST in `docs/peer-session-spec.md`: `core.text`/`core.bin` DataChannel labels for both roles, idle→connecting→connected→closed/failed state machine, `sendText` bounds (state guard, 64 KiB cap, UTF-8 wire), idempotent `close()` with `bye` publication, signaling-message validation (`from`/`to` filtering), peer-initiated `bye`, listener exception isolation |
...
| HTTP-poll endpoint conformance (operator-side) | `pnpm verify:http-poll-endpoint <url>` — black-box probe that drives `@senn/signaling-http-poll` against any deployed Tier-1 endpoint and asserts the spec wire contract (publish/subscribe round-trip + 4xx for malformed roomId / unknown kind). Use after deploying a third-party reference (PHP, Python, Go, Workers, …) |
| HTTP-poll endpoint self-test (in-process) | `pnpm verify:http-poll-self-test` — same probe pointed at the Node reference server (`examples/signaling-http-poll-server/node/server.mjs`) booted on an ephemeral port. Catches probe-script drift, reference-server drift, and adapter contract drift at once. Included in `pnpm conformance` (~1 s) |
